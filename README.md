...
true, the driver returns the cached result set and does not interrogates RedShift. If the query has not been cached yet,
the driver runs the query against RedShift and cache the result set into Redis before returning it. The query itself is used
...
The main idea behind this driver wrapper is that in a lot of scenarios where RedShift is used as a
data warehouse, data is read only and is updated by an ETL process with a pre-defined frequency. In this scenario, it makes
...
You should take care of invalidating the values cached in Redis at the appropriate time (usually after the ETL process has finished).
...
as the statement result set cache key is incorrect. This might not be an issue as long as the key is unique.
* Complex SQL types such as `CLOB`, `BLOB`, `ROWID`, `ARRAY`, etc. are not supported in `PreparedStatement` for caching.
